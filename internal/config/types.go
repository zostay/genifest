...
// MarshalYAML implements yaml.Marshaler for FilesConfig.
// If only Include is set with no custom excludes, marshal as simple string array.
func (fc FilesConfig) MarshalYAML() (interface{}, error) {
	// If it's a simple include-only config with default excludes, marshal as array
	defaultExcludes := []string{"genifest.yml", "genifest.yaml"}
	if len(fc.Include) > 0 && len(fc.Exclude) == 0 {
		return fc.Include, nil
	}
	if len(fc.Include) > 0 && len(fc.Exclude) == len(defaultExcludes) {
		// Check if excludes are the defaults
		isDefault := true
		for i, exclude := range fc.Exclude {
			if exclude != defaultExcludes[i] {
				isDefault = false
				break
			}
		}
...
// UnmarshalYAML implements yaml.Unmarshaler for FilesConfig.
// Supports both string array and map formats.
func (fc *FilesConfig) UnmarshalYAML(value *yaml.Node) error {
	// Try to unmarshal as a simple string array first (legacy format)
	var includes []string
	if err := value.Decode(&includes); err == nil {
		fc.Include = includes
		fc.Exclude = []string{"genifest.yml", "genifest.yaml"} // Default excludes
		return nil
	}
	...
	fc.Include = config.Include
	fc.Exclude = config.Exclude

	// Set default excludes if none specified
	if len(fc.Exclude) == 0 {
		fc.Exclude = []string{"genifest.yml", "genifest.yaml"}
	}
